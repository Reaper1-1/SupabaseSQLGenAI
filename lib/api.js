...
const getApiUrl = () => {
  // Check if running in Replit environment (web only)
  if (typeof window !== 'undefined' && window.location && window.location.hostname && window.location.hostname.includes('replit')) {
    // In Replit, use the external port mapping for 3001
    // Port 3001 is mapped to external port 43751 according to .replit config
    const protocol = window.location.protocol;
    const hostname = window.location.hostname;
    // Replace the port number in the hostname with 43751
    const apiHostname = hostname.replace(/-\d+(-|\.)/g, '-43751$1');
    return `${protocol}//${apiHostname}/api`;
  }
  // Fallback for local development and React Native
